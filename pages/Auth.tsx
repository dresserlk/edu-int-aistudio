...
import { supabase } from '../services/supabaseClient'; // Direct access for auth methods
...
export const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fetch Profile via DataService
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
        // 1. Create Institute Record (This needs to happen first or concurrently depending on your RLS)
        // Actually, better flow:
        // 1. Create Institute -> Get ID
        // 2. SignUp User with Metadata { institute_id: ID, role: 'MANAGER', full_name: ... }
        // 3. Trigger creates Profile
        
        // Creating Institute first (public access allowed in SQL for now)
